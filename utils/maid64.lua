...
---@param common common
function maid64.start(common)
    love.graphics.setCanvas(maid64.canvas)
    love.graphics.clear(common.config.config.colourModes[common.config.config.colour].black)
end
...
function maid64.resize(w, h, fw, fh)
    if maid64.overscan then
        ...
        maid64.right = maid64.sizeX
        maid64.left = 0
        maid64.x = (fw or w)/2 - (maid64.scaler * (maid64.sizeX/2))
    end
    maid64.y = (fh or h)/2 - (maid64.scaler * (maid64.sizeY/2))
end
